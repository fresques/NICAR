...
schools <- schools %>% na.omit() %>% filter(type!="HIGH SCHOOL")# 2892
...
schools %>% select(contains("pct"),enrollment) %>% summary()
```
...
So, in this case, a one-percentage-point increase in low income students is associated with a 0.47 percentage point decrease in proficiency rates. A finding!
...
c <- ggplot(schools,aes(x=pctBlack,y=PARCCpct))
c + geom_point() + geom_smooth(method="lm", se=FALSE) + facet_wrap(~type)
```
